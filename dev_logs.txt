...
That logic would clash heavily with renaming songs, so I decided to make an `identifier` property for the classes, make a function called `createNewId` to efficiently create identifiers for songs and playlists, then finally, scan every single line to change up logic like `if (existingSong.name === newSong.name)` to `if (existingSong.identifier === newSong.identifier)`. This took about 2h and 30m.
I also had to update `validateFiles` to check for duplicates by comparing file name, file size, and file type. I'm aware that this also has it's limits, but it's an improvement in its own right. It was also while doing research on how to better check for file duplicates that I discovered Javascripts ternary operator, which I though was both very cool and very useful, so I replaced a lot of basic if-else statements in my code with it (with the consideration of code-readability).

I finished up all of the logic for `setObjectValues`, the inputs were pretty easy to get working, and the image-replacement functionality didn't take too long either, mainly because of my experience working with audio files at the start of this project. I think I got done this in under 30 minutes.

Just hit one thousand lines of code! I guess that could be both good and bad. I added a backdrop blur to the img-element in the modify menu. It appears when the cursor hovers over it.
I've also started to work on a custom slider for the audio element that syncs with the songs current time. I got the logic working, but getting the styling to look right has been a real pain, it includes working with `-webkit` in CSS which is almost foreign to me.
Right now the slider's design is still very unfinished but I've made a little progress after finding a this website which provides a basic template on how to design input's that are range types "https://codepen.io/ShadowShahriar/pen/zYPPYrQ".
...
The bug was directly related to the newly implemented music visualizer. Similarly to how browsers prevent `audioEl.play()` from playing audio on page load, browsers also suspend `AudioContext` objects on page load, to add to this, the `createMediaElementSource` method of `AudioContext` controls the output generated by the element provided to it via the Web Audio API graph (if `AudioContext` is suspended, the graph doesn't generate any data). When the browser auto-suspended my `AudioContext` object, my audio-element linked to it through `createMediaElementSource` was incapable of creating noise.
The fix was incredibly simple, I just had to create an event listener that unsuspends the `AudioContext` when the user interacts with the page ("click" is the most straightforward listener for this).
...
I've also added a new feature which allows users to drag their songs up and down a playlist. The drag/drop mechanism was no issue, considering all of the other dragging features I had to learn prior to this one, but I did have to research methods for reordering items in an array. For this feature I didn't want songs to swap places when they were moved around, I wanted them to simply "shift" up/down the playlist, while the songs below it shifted up/down by 1. Thankfully there was a very simple and straightforward solution to this which only used `.splice()`.
